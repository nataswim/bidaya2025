...
@section('title', 'Créer une Catégorie Workout')
@section('page-title', 'Nouvelle Catégorie')
@section('page-description', 'Création d\'une nouvelle catégorie de workout')

@section('content')
<div class="container-fluid">
    <form method="POST" action="{{ route('admin.workout-categories.store') }}">
        @csrf
                    ...
                    <div class="card-body p-4">
                        ...
                        <select name="workout_section_id" 
                            ...
                            @foreach($sections as $section)
                                <option value="{{ $section->id }}" {{ old('workout_section_id') == $section->id ? 'selected' : '' }}>
                                    {{ $section->name }}
                                </option>
                            @endforeach
                        </select>
